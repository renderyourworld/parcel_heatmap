...
func StartTaxImporter(gormDB *gorm.DB, countyName string, resume bool, maxParcels int, logPerf bool) error {
	...
	checkpoint, err := initializeCheckpoint(gormDB, countyName, "tax", resume)
	if err != nil {
		return err
	}
	...
	for {
		...
		if err := updateCheckpoint(gormDB, countyName, "tax", int64(currentID), successCount, failCount); err != nil {
	...
	if err := completeCheckpoint(gormDB, countyName, "tax", successCount, failCount); err != nil {
